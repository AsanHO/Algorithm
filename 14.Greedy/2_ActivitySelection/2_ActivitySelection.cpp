...
void Print(vector<Activity>& activities) {
    ...
    for (int i = min_time; i <= max_time; i++) cout << i % 10;
    cout << endl;
...
int main() {
    vector<Activity> activities = {{5, 7}, {2, 5},  {6, 13}, {1, 8},  {6, 9},
                                   {3, 8}, {9, 11}, {5, 12}, {8, 10}, {1, 4},
                                   {3, 6}, {8, 14}, {13, 15}};
...
// CS161 예제에서는 끝나는 시간이 포함이고 CLRS p.422에서는 half-open이라서
// 끝나는 시간 미포함입니다. 여기서는 CS161 방식이 더 직관적이라서 그대로
// 사용하였습니다. 예) {5, 12}이면 12도 활동 시간에 포함