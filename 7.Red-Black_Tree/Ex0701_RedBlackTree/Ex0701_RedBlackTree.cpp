/*
스스로 균형을 잡을 수 있는 이진 탐색 트리로, AVL트리보다 많이 사용된다.
C++ std::map, JAVA TreeMap 같은 내부 자료구조가 레드 블랙 트리를 사용한다.
파이썬의 dict는 해쉬테이블을 사용함

앞의 자료구조에서는 데이터의 값들만 배열의 저장했지만(item,left,right)
여기서는 키와 값을 함께 저장하며, 값에 접근하는 경우 키를 통해야만 한다.

애초에 std::map이 코테 문제를 풀때 굉장히 많이 응용된다.

물론 직접 구현할 일은 없으나, 추상적 그래프 구조를 머리속에 그려내는 능력을 키워준다
이런점에서 알고리듬 학습에서 중요한 역할을 한다.
실무에서 복잡한 자료구조를 다뤄야 하는 상황이 왔을때, 2-3트리와 같이 추상적인 개념이
실제 구현으로 어떻게 연결되는지 공부해본 경험은 큰 도움이 될 것이다.
*/
#include <assert.h>
