...
class IndexMinPQ {
   public:
    IndexMinPQ(int cap) {
        ...
        qp.resize(capacity + 1,
                  -1);  // -1로 초기화(아직 추가되지 않았다는 의미)
    }
    ...
    void Swim(int k) {
        // TODO: 우선순위가 높은 것을 수영해서 수면위로 올라가듯 위로 올리는
        // 과정 힌트: isGreaterA2B(), Exch() 사용
        if (size == 1) return;
        while (!isGreaterA2B(k, k - 1)) {
            Exch(k, k - 1);
            Key temp = keys[k - 1];
            IncreaseKey(k - 1, keys[k]);
            DecreaseKey(k, temp);
        }
    }
    ...
    void Sink(int k) {
        ...
        // 힌트: isGreaterA2B(), Exch() 사용
        if (size == 1) return;
        while (isGreaterA2B(k, k + 1)) {
            Exch(k, k + 1);
            Key temp = keys[k + 1];
            IncreaseKey(k, keys[k + 1]);
            DecreaseKey(k + 1, keys[k]);
        }
    }
    ...
    void Print() {
        using namespace std;
        cout << "Index: ";
        for (int i = 1; i <= size; i++) cout << setw(5) << i;
        cout << endl;

        cout << "Key:   ";
        for (int i = 1; i <= size; i++) cout << setw(5) << keyOf(pq[i]);
        cout << endl;

        cout << "Value: ";
        for (int i = 1; i <= size; i++) cout << setw(5) << pq[i];
        cout << endl << endl;
    }
