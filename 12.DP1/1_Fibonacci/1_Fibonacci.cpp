...
int MemoizedTopDownFibonacciHelper(int n, std::vector<int>& memo) {
    // TODO:
    if (memo[n - 1] == -1) {
        MemoizedTopDownFibonacciHelper(n - 1, memo);
    }
    if (memo[n - 2] == -1) {
        MemoizedTopDownFibonacciHelper(n - 2, memo);
    }
    if (memo[n - 2] != -1 && memo[n - 1] != -1) {
        memo[n] = memo[n - 2] + memo[n - 1];
    }
    for (auto& t : memo) cout << setw(4) << t;
    cout << endl;
...
int BottomUpFibonacciTabulation(int n) {
    ...
    for (int i = 2; i < table.size(); i++) {
        ...
        for (auto& t : table) cout << setw(4) << t;
        cout << endl;
    }
...
int main() {
    int n = 1000;  // 숫자가 크면 단순 재귀 버전은 많이 느립니다.
