...
void Print(const vector<vector<T>>& dist) {
    ...
    for (int i = 0; i < V; i++) {
        for (int j = 0; j < V; j++) cout << setw(5) << dist[i][j];
        cout << endl;
    }
    cout << endl;
}
...
void RepeatingBellmanFord(const vector<vector<double>>& graph) {
    ...
    for (int i = 0; i < V; i++)
        for (int j = 0; j < V; j++) {
            if (i == j || graph[i][j] == kInf)  // 자기 자신이거나 무한대일경우
                prev[i][j] = -1;
            else
                prev[i][j] = i;
        }
    ...
    {
        for (int v = 1; v < V; v++) {
            // CLRS 교재에서는 Floyd-Warshall을 구현할 때 사본을 만들어서
            // 업데이트 방식이라서 여기서는 이해를 돕기 위해 Bellman-Ford도
            // 비슷하게 구조를 바꿔봤습니다. 참고로, 위키피디아에서는 사본 없이
            // 바로 덮어쓰는 in-place 방식입니다.
            auto dist_k = dist;
            auto prev_k = prev;
            cout << "whty?" << endl;
            ...
            for (int i = 0; i < V; i++) {
                for (int j = 0; j < V; j++) {
                    // TODO: grpah[][]와 dist[][]를 사용해서 dist_k와 prev_k
                    // 업데이트
                }
            }
...
int main() {
    vector<vector<double>> graph = {{0.0, 3.0, 8.0, kInf, -4.0},
