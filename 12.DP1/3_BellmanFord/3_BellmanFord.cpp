...
void Print(vector<double>& dist) {
    for (int i = 0; i < dist.size(); i++) cout << setw(6) << dist[i];
    cout << endl;
}
...
int main() {
    ...
    // vector<Edge> edges = {
    //     {0, 1, 10.0}, {0, 3, 5.0}, {1, 2, 1.0}, {1, 3, 2.0}, {2, 4, 4.0}, {3, 1, 3.0}, {3, 2, 9.0}, {3, 4, 2.0}, {4, 0, 7.0}, {4, 2, 6.0},
    // };
    ...
    // vector<Edge> edges = {
    //     {0, 1, 1.0},                                                         // A->B 1
    //     {1, 2, 5.0}, {1, 3, 4.0}, {2, 3, -3.0}, {3, 4, 1.0}, {4, 3, -100.0}  // 음의 싸이클이 추가된 경우
    // };
    // int V = 5;  // number of vertices
    ...
    vector<Edge> edges = {
        {1, 2, 5.0}, {1, 3, 8.0}, {1, 4, -4.0}, {2, 1, -2.0}, {3, 2, -3.0}, {3, 4, 9.0}, {4, 2, 7.0}, {4, 0, 2.0}, {0, 1, 6.0}, {0, 3, 7.0},
    };
    int V = 5;  // number of vertices
    ...
    for (int v = 1; v < V; v++) {
        for (auto e : edges) {
            if (dist[e.w] > dist[e.v] + e.weight) {  // min(현재 거리, 새로 알게된 거리)
