...
using namespace std;
/*
1. 지나온 경로를 기록하기 위해서 prev를 사용합니다.
2. prev[w->value] = v; // w 이전에 방문한 정점이 v라는 것을 기록 (재귀적으로 사용)
3. 그 아래에서 prev를 이용해서 deque<Vertex*> path에 전체 경로를 저장해주면 됩니다.
*/
    ...
    deque<Vertex*> BreadthFirstPaths(int source, int sink) {
        ...
        while (!q.empty()) {
            ...
            for (Vertex* w : v->out_neighbors) {
                if (!w->visited) {
                    w->visited = true;
                    cout << "	" << "	" << "prev[" << w->value << "]에" << v->value << " 저장" << endl;
                    prev[w->value] = v;
                    q.push(w);
                }
            }
...
int main() {
...

    {
        ...
        for (auto& l : jobs) {
            int v = map[l[0]];
            for (int i = 1; i < l.size(); i++) g.AddDiEdge(v, map[l[i]]);
        }

        deque<Vertex*> result = g.BreadthFirstPaths(map["Algorithms"], map["Neural Networks"]);

        for (auto* v : result) cout << values[v->value] << " " << v->value << endl;
    }
    return 0;
    {
        // movies.txt 파일 출처
