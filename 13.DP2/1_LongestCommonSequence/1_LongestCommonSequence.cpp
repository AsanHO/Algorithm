...
int RecurLengthLCS(const string& X, const string& Y, int m, int n) {
    ...
    if (X[m - 1] == Y[n - 1])
        ...
        return max(RecurLengthLCS(X, Y, m - 1, n),
                   RecurLengthLCS(X, Y, m, n - 1));
}
...
int BottomUpLCS(const string& X, const string& Y, int m, int n) {
    ...
    //       table에서 1-base를 사용하는 이유는 i나 j가 0인 base case들도
    //       포함시키기 위해서 입니다.
        ...
        for (int j = 1; j <= n; j++) {
            if (X[i - 1] == Y[j - 1])  // 주의: i - 1, j - 1
            {
                ...
                lcs[i][j] = lcs[i - 1][j - 1] + X[i - 1];
                       ...
                       table[i][j - 1])  // 1,1일때, 상좌표가 좌좌표 이상이면
            {
    ...
    cout << table[m][n] << endl;
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) cout << left << setw(3) << table[i][j];
        cout << endl;
    }
    cout << endl;
    ...
    for (int i = 1; i <= m; i++) {
        for (int j = 1; j <= n; j++) cout << setw(3) << prev[i][j];
        cout << endl;
    }
    cout << endl;
