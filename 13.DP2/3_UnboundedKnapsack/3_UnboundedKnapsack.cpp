...
int RecurUnboundedKnapsack(vector<int> weights, vector<int> values, int W,
                           int n) {
    ...
    if (weights[n - 1] > W)  // 현재 용량 보다 n-1번째 물건의 용량이 큰경우
    {
        return RecurUnboundedKnapsack(weights, values, W, n - 1);
    } else {
        return max(
            RecurUnboundedKnapsack(weights, values, W, n - 1),
            RecurUnboundedKnapsack(weights, values, W - weights[n - 1], n) +
                values[n - 1]);
    }
}
...
int BottomUpUnboundedKnapsack1(vector<int> weights, vector<int> values, int W) {
    ...
    vector<vector<vector<int>>> items(
        W + 1, vector<vector<int>>(weights.size() + 1,
    ...
    for (int w = 1; w <= W; w++) {
        for (int n = 1; n <= weights.size(); n++) {
            if (weights[n - 1] > w)  // 아이템 n의 무게가 넣을 수 있는 무게인지
            {
                ...
                // if (table[w][n - 1] > table[w - weights[n - 1]][n - 1] +
                // values[n - 1]) // ZeroOne
                if (table[w][n - 1] >
                    table[w - weights[n - 1]][n] + values[n - 1]) {
                    table[w][n] = table[w][n - 1];
                    items[w][n] = items[w][n - 1];
                } else  // Case 2: 넣는 경우
                {
...
int BottomUpUnboundedKnapsack2(vector<int> weights, vector<int> values, int W) {
    ...
    for (int w = 1; w <= W; w++) {
        ...
        cout << "Weight " << setw(2) << w << ", " << "Value " << setw(2)
             << table[w] << ", items: ";
        Print(items[w]);
    }
...
int main() {
...

    {
        ...
        cout << RecurUnboundedKnapsack(weights, values, W, int(weights.size()))
             << endl;
        cout << endl;
    }
    // return 0;
