   ...
   public:
    ...
    void ConnectedComponents() {
        ...
        for (Vertex* v : vertices) {
            if (!v->visited) {  // 방문하지 않았다면 탐색시작
                DFS(v);
                count++;
            }
        }
        // 결과 정리 후 출력
        cout << "탐색 종료, 컴포넌트 개수 = " << count << endl;
...
int main() {
    // Sedgewick p544
    // 무방향(양방향) 그래프
    vector<vector<int>> edges = {{0, 5}, {4, 3}, {0, 1}, {9, 12}, {6, 4}, {5, 4}, {0, 2}, {11, 12}, {9, 10}, {0, 6}, {7, 8}, {9, 11}, {5, 3}};

    Graph g(13);

    for (vector<int>& p : edges) g.AddBiEdge(p[0], p[1]);
