...
class Graph {
   public:
    ...
    bool HasPath(int start, int target) {
        for (auto* v : vertices) v->visited = false;  // 방문초기화
    ...
    void BruteForceStrongComponents() {
        ...
        for (int i = 0; i < vertices.size(); i++) {
            if (id[i] != -1) continue;  // id에 인덱스가 -1이 아니라면 탐색이 완료된 경우
            id[i] = count;
            for (int j = 0; j < vertices.size(); j++) {
                if (i != j) {
                    cout << i << "와" << j << "연결 여부" << (HasPath(i, j) == HasPath(j, i)) << endl;
        ...
        for (int s = 0; s < vertices.size(); s++) components[id[s]].push_back(s);
   ...
   private:
    ...
    bool HasPathHelper(Vertex* v, Vertex* t) {
        ...
        for (Vertex* v_out : v->out_neighbors) {
            if (HasPathHelper(v_out, t)) return true;
        }
        return false;
    }
};
...
int main() {
    // Sedgewick p569
    // 방향(directed) 그래프
    vector<vector<int>> edges = {{4, 2},   {2, 3},  {3, 2}, {6, 0}, {0, 1}, {2, 0}, {11, 12}, {12, 9}, {9, 10}, {9, 11}, {7, 9},
                                 {10, 12}, {11, 4}, {4, 3}, {3, 5}, {6, 8}, {8, 6}, {5, 4},   {0, 5},  {6, 4},  {6, 9},  {7, 6}};

    Graph g(13);

    for (vector<int>& p : edges) g.AddDiEdge(p[0], p[1]);
